WITH CitiesOrdered AS (
    ...
    INNER JOIN contitravel.dbo.vettori as v
...
SELECT
    CAST(concat('C-',t.Tkt_id) AS NVARCHAR(50)) AS id,
    CAST(t.Tkt_NTkt AS NVARCHAR(15)) AS number,
    CAST(concat('C-',t.Tkt_cli) AS NVARCHAR(10)) AS clientcode,
    ...
    CAST(concat('C-',t.Tkt_codoperatore) AS NVARCHAR(15)) AS operator,
...
LEFT JOIN CitiesOrdered c
    ON CAST(concat('C-',t.Tkt_id) AS NVARCHAR(50)) = c.Tkt_id
GROUP BY
    t.Tkt_id,
    t.Tkt_NTkt,
    t.Tkt_cli,
